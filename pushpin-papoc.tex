...
\acmPrice{15.00}
\acmDOI{10.1145/3380787.3393683}
...
When peers are on the same LAN (wired or wireless network), they attempt to discover one another using mDNS, a variation on DNS that broadcasts DNS-like service advertisements or discovery requests to a well known multicast IP address~\cite{RFC6762}.
...
% While a STUN server is only involved during connection establishment, a TURN server needs to proxy all network traffic between the peers, making it significantly more expensive to operate.
...
We synchronise application state between replicas using the Automerge CRDT, and we synchronise local application state with the user interface using the Document FRP approach. This programming model treats all state updates equally, regardless of whether they originated locally or on a remote device, eliminating ad-hoc calls to remote APIs and callback hell. Updates to the CRDT can be replicated immediately for real-time collaboration, or batched up and sent asynchronously if desired. We achieved high performance by splitting Automerge into a frontend and backend, running expensive operations on a background thread without blocking the user interface thread.
