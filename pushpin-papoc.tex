...
The web browser has become the de-facto standard for cross-platform development. It has a robust ecosystem of programming languages (e.g.\ TypeScript), frameworks (especially React~\cite{React}) and utility libraries (e.g.\ PDF rendering~\cite{PDFjs}). The browser continues to become a more powerful execution and rendering environment, with new features for HTML and CSS always evolving. The browser is also home to perhaps the largest community of developers. Thus, by building PushPin within this ecosystem, we gain access to these tools, and hope to remain accessible to a large community of developers.
...
\item[Storage.]
By default, browsers do not reliably store data or software. The Progressive Web Application standard, and APIs like localStorage or IndexedDB provide some functionality to preserve local copies of software and preserve data locally, but these systems are slow, and worse, often silently lose user data. These APIs appear to be built with the assumption that user data is a local cache, and not authoritative, and are prone to being expired without warning or notification~\cite{LocalStorageCleared}.
...
Loss of user data is a cardinal sin, and the fact that browsers are unreliable keepers of locally persisted data is disqualifying for local-first implementations. Electron, on the other hand, provides full access to the users' local file system through a variety of approaches.
...
Each card has \texttt{x}, \texttt{y}, \texttt{width} and \texttt{height}  attributes recording its dimensions, and a \texttt{url} linking to the document that stores its content. The URL also includes a "contentType" attribute, which informs the application which code tot select to render the document.
...
The most rudimentary approach to DFRP is to keep all application state in a single CRDT. This is problematic: for some types of state, such as unsubmitted text in HTML textboxes, or page scroll positions, each PushPin instance should have its own state~--- making all replicas converge to the same state is undesirable in these cases.
...
Monolithic FRP can become difficult to reason about, as all messages and states are unified into a single transition function. The PushPin project associates independent FRP functions with individually replicable CRDTs as its fundamental primitive. These can be nested arbitrarily to produce complex views.

For example, the board document \emph{render} function can include a mixture of text or image documents beneath it. Likewise, the same FRP code could render the text content at the root of the application to show the content full-screen. This makes the user interface components more easily reusable and composable.
...
For peer discovery it uses a combination of mDNS for local peer discovery, and a DHT with seed nodes provided by a nonprofit foundation, and it uses BitTorrent's uTP protocol to establish connections.
\end{description}
...
When peers are not on the same LAN, the Dat protocol queries a Distributed Hash Table~\cite{DHT} for peer discovery. A DHT operates by discovering new peers during its operation, but must have a few "bootstrapping" nodes hard-coded into the distribution. In PushPin's case, the bootstrapping nodes are operated by the Dat non-profit organisation. In practice, this is a form of weak centralisation, as if all of those nodes become unavailable, a software update would be necessary, but in principle there is no reason further work could not go into distributing those nodes more resiliently, i.e. across multiple organisations. 
...
However, mobile devices often use networks with NAT on which UPnP is not available, such as a coffee shop WiFi, a corporate office network.
...
In the case of PushPin, users in this position can take advantage of indirect replication through another mutually routable client. In fact, this is one of the key benefits of running a storage peer described in the next section. Note that while it is not implemented in PushPin today, in principal, any clients could provide relay services to other peers. 
...
For example, if your colleague has shared the URL of a PushPin board with you, it would be annoying if you could not access that board because the only copy of the board resides on your colleague's laptop, and that laptop's lid is currently closed, or the application is currently closed.
...
\begin{itemize}
    \item respond quickly to local user input (ideally, <15ms)
    \item permit local writes at all times
    \item allow collaboration over all network connections (with or without internet)
    \item not rely on any centralised services
\end{itemize}

We believe that replicating data stored in the Automerge CRDT over a peer-to-peer system based on Dat, interacting with the data through a React FRP rendering engine and delivering it to clients as an Electron application meets these goals.
